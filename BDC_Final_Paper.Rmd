...
ggplot(data = team_plot_df,aes(x_coord,y_coord)) +
  ...
  #geom_density_2d(aes(alpha=..level..),color="light grey") +
...
tab_function <- function(data, ...){
  data %>% 
    ...
    cols_label(
      ...
      mcmc.sd = "DPAA Sdev",
      sim.mean = "DPAA Sim",
      sim.sd = "DPAA Sim Sdev",
      closest_opp_player_separation = "Avg. Defender Separation (m)",
      Pass = "Pass Plays",
      Pass_PDAA = "Pass DPAA"
    ) %>% 
    data_color(
...
ggplot(data = new_plot,aes(x_coord,y_coord)) +
  gg_rink(side="right") +
  stat_density_2d(aes(fill=DPAA_group,alpha=(..level..)^2),geom="polygon",size=3) +
  geom_density_2d(aes(alpha=..level..),color="light grey") +
  ...
  scale_fill_manual(values = c("red", "orange",
                               "light blue", "blue",
                               "green")) +
  facet_wrap(~closest_defensive_player,ncol = 3) +
    theme_void() +
    theme(panel.grid = element_blank(),
        axis.line = element_line(),
        axis.ticks=  element_blank(),
  ...
  labs(x = "",
       ...
       subtitle = "Shows concentration of plays  made | DPAA ",
       legend = "DPAA",
       fill = "DPAA",
...
--- List key action points and conclusion
--- List application to other aspects
